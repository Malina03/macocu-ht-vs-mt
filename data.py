...
def load_corpus_balanced_mt(args, phase, split_docs_by_sentence=False):
    ...
    if phase not in ("train", "dev"):
        raise ValueError("Phase should be one of 'train' or 'dev. There is no need to balance testing data.'")
...
def load_corpus(args, phase, split_docs_by_sentence=False):
    ...
    mt = args.mt if args.mt else "google"
    apdx = args.test_folder if args.test_folder else ""
    ...
    paths = {
        ...
        1: (
            ...
            + list((root_dir / f"data/{mt}/{phase}/{apdx}").glob("*.en.google"))
            + list((root_dir / f"data/{mt}/{phase}/{apdx}").glob("*.opus.en"))
            + list((root_dir / f"data/{mt}/{phase}/{apdx}").glob("*.wmt"))
        ),
    }  # all the text files per class
...
def load_sentence_pairs(args, phase, split_docs_by_sentence=False):
    """
    ...
    _mt_suffixes = [".txt.en.google", '.deepl.en', ".opus.en", 'wmt']
    ...
    if phase == "test":
        test_folder = args.test_folder if args.test_folder else ""
        paths = {
        ...
        paths = {
           # adapted for translationese
            0: (list((root_dir / f"data/{mt}/{phase}/").glob("trans_??en_en_wmt??.txt")) 
                + list((root_dir / f"data/{mt}/{phase}/").glob("org_en??_en_wmt??.txt"))),
            1: list(
                itertools.chain.from_iterable(
    ...
    for label, path_lst in paths.items():
        for path_B in path_lst:
            ...
            lang = re.search(r"(de|ru|zh)", path_B.name).group(1)
             # Match translation from original text to the source text.
            if path_B.name in [
                ...
                f"org_{lang}en_{lang}_wmt{wmt_year}.wmt"
            ]:
            ...
            # Match translationese to the original text. There is no translationese for translations by wmt systems. 
            elif path_B.name in [
...
def load_sentence_pairs_balanced_mt(args, phase, split_docs_by_sentence=False):
    """
    Loads data from disk, where instead of individual sentences, bilingual sentence
    pairs are loaded (German-English/Russian-English/Chinese-English).
    ...
    """
    if phase not in ("train", "dev"):
        raise ValueError("Phase should be one of 'train' or 'dev'. There is no need to balance the test data.")
